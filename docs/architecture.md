...
| **Cron execution** | Registered cron scripts fire on schedule, independent of Player PTY lifetime. Output written to Maestro-assigned scratchpad. CronFired event enqueued to owning Player. |
...
| Concern | Details |
|---|---|
| **Layout position of any pane** | Including the Conductor's own pane. The Conductor calls `PUT /layout` at boot with whatever tree it wants. Maestro renders it — no defaults, no enforcement. |
...
- *"Cron scripts run on schedule independent of Player PTY lifetime"* → yes, always true → Maestro owns it.
